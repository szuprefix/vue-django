<script setup>
    ...
    import {ref, reactive, defineAttrs, onMounted, onBeforeDestroy, computed, watch} from 'vue'
    attrs = defineAttrs()
...
//            parentRelationQueryName: String,
    ...
    const model = reactive(Model(this.appModel, {}, this.$store.state.bus))
    const tableItems = ref([])
    const search = reactive({})
    const optionLoaded = ref(false)
    const creator = reactive(undefined)
    const editing = ref(false)
    const batchActionItems = ref([])
    const selection = ref([])
    const parentQueries = reactive({})
    const count = ref(0)
    const avairableActions = reactive({
        'create': {
            icon: 'plus',
            title: '创建',
    ...
    function onSearch(qd) {
        this.search = {...qd}
        console.log(qd)
        this.$refs.table.updateQueries({...qd, page: 1})
    }
    function onModelPosted({appModel, id}) {
    ...
    function toEditModel({row}) {
        this.$router.push(this.model.getDetailUrl(row.id))
    }

    function toDeleteModel({row}) {
    ...
    function defaultBatchActionDo(action) {
        return ({selection, scope, confirmResult}) => {
            ...
            return this.$http.post(`${this.model.getListUrl()}${action.api || action.name}/?${Qs.stringify(qd, {
                arrayFormat: 'comma',
                skipNulls: true
            })}`, pd).catch(this.onServerResponseError)
        }
    }
    function addToParent({selection}) {
        let d = {}
    ...
    function genDefaultFormatter(f) {
        if (f.choices) {
...
//                        return r
    ...
    function addSearchParentQueries(d) {
    ...
    const parentMultipleRelationField = computed(
        () => {
            if (this.parent) {
