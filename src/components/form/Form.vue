<script setup>
    import {reactive, computed, ref, useAttrs} from 'vue'
    ...
    formItems.value = Form.normalizeItems(props.items)
    function doSubmit() {
    ...
    function onValidated(valid, notValidFields) {
        if (valid) {
            this.errors = {}
        }
    }
...
<template>hello
    <el-form ref="form" :model="formValue" v-if="modelValue"
        ...
        <el-row>
            <template v-for="f in formItems">
                <template v-if="!f.hidden">
                    {{f}}
                    <!--<el-col v-bind="[f.span]" :key="f.name"-->
                            <!--v-if="!$attrs.inline && !$attrs.oneColumn && f.widget !== 'hidden'">-->
